...
- **`src/schema.sql`**: two tables — `players` (name, `elo`, starts at 1200) and `matches` (two player FKs, `sets` as `JSONB`, `winner_id`, each player's post-match ELO). Applied via `npm run migrate` (`src/migrate.ts`, which just splits the file on `;` and runs each statement), not a migration framework — schema changes mean editing `schema.sql` and re-running migrate. Because `CREATE TABLE IF NOT EXISTS` won't alter an existing table, changes to existing tables are also added as idempotent `ALTER TABLE ... IF NOT EXISTS`/`DROP CONSTRAINT IF EXISTS` statements below the CREATEs (see the unused `deleted_at` column and the partial unique index on names). Run migrate against production *before* deploying API code that depends on the change.
...
- **Deleting players**: `DELETE /api/players/:id` runs in one transaction — it deletes the player's matches, deletes the player, then calls `replayRatings` (`src/ratings.ts`), which rebuilds **every** player's ELO and every match's `*_elo_after` by replaying the remaining matches in `id` order (the order ratings were originally applied, *not* `played_on` order) from 1200. That's required because ELO is a running total: later matches were computed on top of the deleted ones. Replay is idempotent, so removing a player with no matches leaves everyone's ratings unchanged. **Individual matches still can't be deleted or edited** — that would go through the same `replayRatings` path. Any future feature that removes or changes a match must call it inside its transaction.
