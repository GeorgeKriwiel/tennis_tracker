...
- **Deleting players**: `DELETE /api/players/:id` hard-deletes a player with no matches (freeing the name), but **soft-deletes** (`deleted_at = NOW()`) one with match history — other players' ELO was computed against them, so their matches must stay. Soft-deleted players are hidden from `GET /api/players` and rejected by `POST /api/matches`, but still appear by name in `GET /api/matches`. Name uniqueness is a partial unique index on active players only, so a removed name can be re-used. **Matches still can't be deleted or edited** — ELO is a running total and later matches were computed on top, so that would mean recomputing ELO forward from the edited point, not just an SQL delete.
